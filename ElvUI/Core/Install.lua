...
local function SetupCVars(noDisplayMsg)
	pcall(SetCVar, "showTooltipID", 1)
	pcall(SetCVar, "mapQuestDifficulty", 1)
	pcall(SetCVar, "ShowClassColorInNameplate", 1)
	pcall(SetCVar, "screenshotQuality", 10)
	pcall(SetCVar, "chatMouseScroll", 1)
	pcall(SetCVar, "chatStyle", "classic")
	pcall(SetCVar, "WholeChatWindowClickable", 0)
	pcall(SetCVar, "ConversationMode", "inline")
	pcall(SetCVar, "showTutorials", 0)
	pcall(SetCVar, "showNewbieTips", 0)
	pcall(SetCVar, "showLootSpam", 1)
	pcall(SetCVar, "UberTooltips", 1)
	pcall(SetCVar, "threatWarning", 3)
	pcall(SetCVar, "alwaysShowActionBars", 1)
	pcall(SetCVar, "lockActionBars", 1)
	pcall(SetCVar, "SpamFilter", 0)
...
local function SetPage(PageNum)
	...
	elseif PageNum == 8 then
		...
		f.Desc1:SetText(L["You are now finished with the installation process. If you are in need of technical support please visit us at https://github.com/RosemyneH/ElvUI_Astral."])
