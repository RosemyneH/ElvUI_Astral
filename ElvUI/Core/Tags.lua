...
local IsInGroup = GenerateClosure(C_Player.IsInGroup, C_Player)
...
local C_QuestLog_GetQuestDifficultyLevel = function(questID)
	...
	if questIndex then
    	return select(2, GetQuestLogTitle(questIndex))
	end
end
...
local function GetClassPower(Class)
	...
	if barType then
		...
		if min > 0 then
			...
			local color = (dk and ElvUF.colors.class.DEATHKNIGHT) or power
			r, g, b = color.r, color.g, color.b
		end
	else
		min = GetComboPoints("player", "target")
		max = 5
...
for textFormat in pairs(E.GetFormattedTextStyles) do
	...
	if tagFormat ~= 'percent' then
		...
		E:AddTag(format('mana:%s:shortvalue', tagFormat), 'UNIT_POWER_FREQUENT UNIT_MAXPOWER', function(unit)
			return E:GetFormattedText(textFormat, UnitPower(unit, 0), UnitPowerMax(unit, 0), nil, true)
		end)
	end
end
