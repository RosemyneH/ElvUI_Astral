...
AddOn.AwesomeNameplates = not not _G.ELVUI_HAS_AWESOME_NAMEPLATES
...
function AddOn:OnInitialize()
	...
	if EscapeMenu and EscapeMenu.AddButton and EscapeMenuSection then
		EscapeMenu:AddButton(self.title, EscapeMenuSection.AddOns, function()
	...
	elseif GameMenuFrame then
		...
		GameMenuFrame:HookScript("OnShow", function(menu)
			...
			local anchor = _G.GameMenuButtonAddons or _G.GameMenuButtonMacros or _G.GameMenuButtonOptions
			GameMenuButton:SetPoint("TOPLEFT", anchor, "BOTTOMLEFT", 0, -1)
			GameMenuButtonLogout:ClearAllPoints()
			GameMenuButtonLogout:SetPoint("TOPLEFT", GameMenuButton, "BOTTOMLEFT", 0, -1)
			if not menu.elvuiHeightAdjusted then
				menu:SetHeight(menu:GetHeight() + GameMenuButton:GetHeight() + 1)
				menu.elvuiHeightAdjusted = true
			end
		end)
	end
