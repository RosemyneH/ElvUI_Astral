...
local IsSpellKnownOrOverridesKnown = CA_IsSpellKnown
...
function mod:StyleFilterDispelCheck(frame, filter)
	local index = 1
	local name, _, _, _, debuffType, _, _, _, isStealable = UnitAura(frame.unit, index, filter)
	while name do
		...
		index = index + 1
		name, _, _, _, debuffType, _, _, _, isStealable = UnitAura(frame.unit, index, filter)
	end
end
...
function mod:StyleFilterAuraData(frame, filter)
	...
	local index = 1
	local name, _, _, count, _, _, expiration, source, _, _, spellID = UnitAura(frame.unit, index, filter)
	while name do
		...
		info[index] = { count = count, expiration = expiration, source = source }

		index = index + 1
		name, _, _, count, _, _, expiration, source, _, _, spellID = UnitAura(frame.unit, index, filter)
	end
				...
				for _, data in pairs(info) do -- need to loop for the sources, not all the spells though
					if not stacks or (data.count and data.count >= stacks) then
						...
						if fromMe and fromPet and (isMe or isPet) or (fromMe and isMe) or (fromPet and isPet) or (not fromMe and not fromPet) then
							local timeLeft = (hasMinTime or hasMaxTime) and data.expiration and (data.expiration - now)
...
function mod:StyleFilterSetupFlash(FlashTexture)
	...
	local fadein1 = anim:CreateAnimation('ALPHA')
	fadein1:SetChange(-1)
	fadein1:SetOrder(3)
	anim.fadein1 = fadein1

	local fadein2 = anim:CreateAnimation('ALPHA')
	fadein2:SetChange(1)
	fadein2:SetOrder(4)
	anim.fadein = fadein2

	local fadeout1 = anim:CreateAnimation('ALPHA')
	fadeout1:SetChange(1)
	fadeout1:SetOrder(1)
	anim.fadeout1 = fadeout1

	local fadeout2 = anim:CreateAnimation('ALPHA')
	fadeout2:SetChange(-1)
	fadeout2:SetOrder(2)
	anim.fadeout = fadeout2
...
function mod:StyleFilterSetChanges(frame, actions, HealthColor, PowerColor, Borders, HealthFlash, HealthTexture, Scale, Alpha, NameTag, PowerTag, HealthTag, TitleTag, LevelTag, Portrait, NameOnly, Visibility)
	...
	if HealthFlash then
		...
		anim.fadein:SetChange(fc.a or 1)
		anim.fadeout1:SetChange(fc.a or 1)
...
function mod:StyleFilterClearVisibility(frame, previous)
	...
	if (previous == 1 or previous == 3) and (state ~= 1 and state ~= 3) then
		...
		frame:Point('CENTER', WorldFrame, 'BOTTOMLEFT', frame.x, frame.y)
	end
		...
		for filterName, filter in pairs(E.global.nameplates.filters) do
			...
			if t and mod.db.filters[filterName] and mod.db.filters[filterName].triggers and mod.db.filters[filterName].triggers.enable then
				...
				if t.inParty or t.notInParty or t.inRaid or t.notInRaid or t.unitRole then
					events.RAID_ROSTER_UPDATE = 1
					events.PARTY_MEMBERS_CHANGED = 1
				end
				...
				if t.isCharmed or t.notCharmed or t.isPossessed or t.notPossessed then
