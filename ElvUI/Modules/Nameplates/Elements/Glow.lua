...
local function TargetArrowTexture()
	local arrows = E.Media.Arrows
	local key = NP.db.units.TARGET.arrow
	return (arrows and (arrows[key] or arrows.ArrowUp)) or E.Media.Textures.ArrowUp
end
...
function NP:Configure_Glow(frame)
	...
	if glowStyle ~= "none" then
		...
		-- Indicators
		frame.LeftIndicator:SetTexture(TargetArrowTexture())
		frame.LeftIndicator:SetVertexColor(r, g, b)
		frame.LeftIndicator:SetSize(arrowSize, arrowSize)

		frame.RightIndicator:SetTexture(TargetArrowTexture())
		frame.RightIndicator:SetVertexColor(r, g, b)
		frame.RightIndicator:SetSize(arrowSize, arrowSize)

		frame.TopIndicator:SetTexture(TargetArrowTexture())
		frame.TopIndicator:SetVertexColor(r, g, b)
