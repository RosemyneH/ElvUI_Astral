...
S:AddCallbackForAddon("Blizzard_DebugTools", "Skin_Blizzard_DebugTools", function()
	...
	if _G.ErrorHandler then
		ErrorHandler:StripTextures()
		if ErrorHandler.NineSlice then
		...
		if ErrorHandlerErrorScrollBar then
			S:HandleScrollBar(ErrorHandlerErrorScrollBar)
			if ErrorHandlerErrorScrollBarScrollDownButton then
				ErrorHandlerErrorScrollBarScrollDownButton:Point("TOP", ErrorHandlerErrorScrollBar, "BOTTOM", 0, 2)
			end
		end

		if S.HandleTabSystem then
			S:HandleTabSystem(ErrorHandler)
		end
		if ErrorHandlerPreviousButton then S:HandleButton(ErrorHandlerPreviousButton) end
		if ErrorHandlerCopyButton then S:HandleButton(ErrorHandlerCopyButton) end
		if ErrorHandlerNextButton then S:HandleButton(ErrorHandlerNextButton) end
		if ErrorHandlerCloseButton then S:HandleCloseButton(ErrorHandlerCloseButton) end
		if ErrorHandlerErrorInset then ErrorHandlerErrorInset:StripTextures() end
	end
