...
function B:UpdateSlotAppearance(slot, clink)
	...
	slot.isUnlearnedVanity = false
	slot.isUnlearnedWardrobe = false
	if slot.id and _G.VANITY_ITEMS and C_VanityCollection and C_VanityCollection.IsCollectionItemOwned then
	...
	if slot.id and C_Appearance and C_Appearance.GetItemAppearanceID and C_AppearanceCollection and C_AppearanceCollection.IsAppearanceCollected then
...
function B:CreateSlotAppearanceElements(slot)
	...
	if not slot.unlearnedWardrobeIcon then
		...
		unlearnedWardrobeIcon:SetTexture(E.Media.Textures.BagVanityAndWardrobe)
