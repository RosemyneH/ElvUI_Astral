...
local function OnEnter(self)
	...
	local crit = GetSpellCritChance(6) or 0
	DT.tooltip:AddDoubleLine("Shadow Crit Chance", format("%.2f%%", crit), 1, 1, 1, 1, 1, 1)
	DT.tooltip:AddLine(format("Crit rating %d (+%.2f%% crit chance)", rating, bonus), 1, 0.82, 0)
	DT.tooltip:Show()
end

local function ValueColorUpdate(hex)
	displayString = join("", "Shadow Crit: ", hex, "%.2f%%|r")
...
DT:RegisterDatatext("Shadow Crit", {"COMBAT_RATING_UPDATE", "UNIT_STATS", "PLAYER_DAMAGE_DONE_MODS"}, OnEvent, nil, nil, OnEnter, nil, "Shadow Crit")
